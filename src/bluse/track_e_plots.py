...
def fig_ablation(rep, path):
    """
    Ordered by score, with the two GAPS labelled rather than the winner.

    The obvious version of this chart -- highlight the stamp row, note it beats
    the flag row by 0.053 -- misattributes 90% of that gap. Track A's flags are
    thresholds on quantities the metadata set holds continuously, and undoing
    the binarisation is worth 0.047 of the 0.053. Morphology adds 0.005 on top.
    A chart that shows only the endpoints invites exactly the wrong reading, so
    this one draws the steps.
    """
    plt = _rc()
    ...
    for i, v in enumerate(vals):
        ...
        ax.plot([0.90, v], [i, i], "-", color=c, linewidth=2,
                alpha=0.5 if i == hero else 0.35, zorder=2)
    ...
    # The two steps that make up the gap, drawn where they happen.
    steps = [(0, 1, "stop binarising", ORANGE),
             (1, 2, "use the pixels instead", BLUE)]
    for lo, hi, text, col in steps:
        ymid = lo + 0.5
        # Head at the HIGH value: the arrow reads as the direction of the gain.
        ax.annotate("", xy=(vals[hi], ymid), xytext=(vals[lo], ymid),
                    arrowprops=dict(arrowstyle="-|>", color=col, linewidth=1.4,
                                    shrinkA=0, shrinkB=0, mutation_scale=11))
        d = vals[hi] - vals[lo]
        ax.annotate(f"{text}   +{d:.4f}",
                    ((vals[lo] + vals[hi]) / 2, ymid),
                    textcoords="offset points", xytext=(0, 7), ha="center",
                    va="bottom", fontsize=9.5, color=col, fontweight="bold")

    # A footnote, kept clear of the gap labels that sit between the rows.
    ax.text(0.9015, -0.95,
            "stamp morphology is the shipped default even though it is not the "
            "highest:\nit cannot relearn the RFI frequency mask",
            ha="left", va="bottom", fontsize=9, color=MUTED, linespacing=1.4)
    ax.set_yticks(y)
    ax.set_yticklabels(labels, fontsize=10)
    ax.set_ylim(-1.05, len(vals) - 0.35)
    ...
    ax.set_title("Where the information is — and it is mostly not the pixels")
    fig.tight_layout()
    fig.savefig(path, bbox_inches="tight")
    plt.close(fig)
    return path
