...
`eom`, and the reason is `f02`. The deferred scaling work and the deferred
`f02` rework are **coupled and ordered**: `f02` must be fixed first.
...
2. **`f02`'s ordinal rework is a PREREQUISITE, not a parallel item.** Equalising
   a column with a 26.6% tie amplifies the tie. Reorder: fix `f02`, then
   equalise, then re-measure.
...
help, under `leaf`, once `f02` is not in the way.
